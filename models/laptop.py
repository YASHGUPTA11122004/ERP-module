...
class Laptop(models.Model):
    ...
    laptop_maintenance_ids = fields.One2many(
        comodel_name="it_infra.laptop_maintenance",
        inverse_name="laptop_id",
    ...
    def create(self, vals):
        ...
        if vals.get('reference_number', 'New') == 'New':
            vals['reference_number'] = f"LPT-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        return super(Laptop, self).create(vals)
    ...
    def write(self, vals):
        ...
        return super(Laptop, self).write(vals)
